...
| `thumbnail` | string | 否 | - | 缩略图，放在 `public/` 下 |
...
如果你希望将文章内容和站点代码分开管理（比如方便多设备写作、或多人协作），可以使用独立的内容仓库。

### 创建内容仓库

1. 新建一个 Git 仓库（如 `my-blog-content`）
2. 将 `content-template/` 目录中的文件复制过去
3. 按需添加文章到 `posts/` 目录
...
项目根目录的 `scripts/fetch-content.sh` 会在构建前自动 clone 内容仓库到 `src/content/`。

> 本地开发时保留 `src/content/posts/` 内的文章不受影响，两边互不冲突。
